...
summarize_matrix <- function(x, na.rm=FALSE) {
  y <- data.frame(mean=c(apply(x, 1, mean, na.rm = TRUE)),
                  stdev=c(apply(x, 1, sd, na.rm = TRUE)),
                  median=c(apply(x, 1, median, na.rm = TRUE)),
                  min=c(apply(x, 1, min, na.rm = TRUE)),
                  max=c(apply(x, 1, max, na.rm = TRUE)),
                  num_lt_0=c(apply(x, 1, function(row) sum(row <0, na.rm=na.rm))),
                  num_btw_1_and_5=c(apply(x, 1, function(row) sum(row > 1 & row < 5, na.rm = na.rm))),
                  num_na=c(apply(x, 1, function(row) sum(is.na(row)))))
  return(y)
}

summarize_matrix_na <- function(x, na.rm=FALSE) {
  y <- data.frame(mean=c(apply(x, 1, mean, na.rm = F)),
                  stdev=c(apply(x, 1, sd, na.rm = F)),
                  median=c(apply(x, 1, median, na.rm = F)),
                  min=c(apply(x, 1, min, na.rm = F)),
                  max=c(apply(x, 1, max, na.rm = F)),
                  num_lt_0=c(apply(x, 1, function(row) sum(row <0))),
                  num_btw_1_and_5=c(apply(x, 1, function(row) sum(row > 1 & row < 5))),
                  num_na=c(apply(x, 1, function(row) sum(is.na(row)))))
  return(y)
}
